...
int main(int argc, char **argv)
{
    ...
    int cellsize = 1;
    int n_grid = 4;  // @ANDY: change back to n_grid=50
    ...
    for (int i = 0; i < totalNumberofTimeStep; ++i)
    {
        ...
        if (rank == 0&& i == 10)
        {
            printf("recv rank before recving\n");
            x = l_grid+1;
            for (int y = 1; y < l_grid+1; ++y)
            {
                printf("%lf ", U[ ((x)*(l_grid+2) + (y))*3 ]);
    ...
    // }
        ...
        if(mey != 0)    
        {
            ...
            {
                // @ANDY:tested
                printf("rank= %i,\tmey= %i,\tmex= %i,\ttime= %i,\ty= %i,\tx= %i\n",rank,mey,mex,i,y,x); //@ANDY:debug
        ...
        if (rank == 0 && i == 10)
        {
            printf("recv rank after recving\n");
            x = l_grid+1;
            for (int y = 1; y < l_grid+1; ++y)
            {
                printf("%lf ", U[ ((x)*(l_grid+2) + (y))*3 ]);
